...
function fnSignature(abi: Func): string {
  const name = transformToFullName(abi)
  return sha3(name).slice(0, 8)
}


export function encodeFunction(abi: Func, args: Array<any>): string {
...
export function encodeConstructor(abi: Func, args: Array<any>, bytecode: string): string {
  const abiInputs = abi.inputs.map(arg => arg.type);
  return bytecode + convert.bytesTB(coder.rawEncode(abiInputs, args))
}

export function decode(abi: Func, output: Uint8Array): DecodedResult {
  if (!output) return
...
export async function callFunction<T>(input: Address, abi: Func, pipe: Pipe, handler: Handler,
...
export async function callConstructor<T>(input: Address, abi: Func, pipe: Pipe, handler: Handler,
